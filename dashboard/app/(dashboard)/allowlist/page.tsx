...
export default function AllowlistPage() {
  ...
  const fetch = useCallback(() => api.allowlist(), []);
  ...
  async function handleAdd() {
    ...
    try {
      await api.allowlistAdd(input.trim());
      setInput("");
      refetch();
    } catch (e) {
  ...
  async function handleRemove(entry: string) {
    try {
      await api.allowlistRemove(entry);
      refetch();
    } catch (e) {
      console.error(e);
    }
  ...
  const ips  = data?.ips          ?? [];
  const nets = data?.cidrs        ?? [];
  const uas  = data?.user_agents  ?? [];

  return (
    <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-3">
      <h1 className="text-[13px] font-medium text-zinc-100">Allowlist</h1>
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-4 flex flex-col gap-3">
        ...
        <div className="text-[10px] text-zinc-500">IP address, CIDR range, or user-agent substring</div>
        <div className="flex gap-2">
          <input
            value={input}
            onChange={e => setInput(e.target.value)}
            onKeyDown={e => e.key === "Enter" && handleAdd()}
            placeholder="1.2.3.4  or  10.0.0.0/8  or  Googlebot"
      ...
      {[{ label: "IPs", entries: ips }, { label: "CIDRs", entries: nets }, { label: "User agents", entries: uas }]
        .filter(g => g.entries.length > 0)
        .map(group => (
          <div key={group.label} className="bg-zinc-900 border border-zinc-800 rounded-lg overflow-hidden">
            <div className="px-3 py-2 border-b border-zinc-800 text-[11px] font-medium text-zinc-400">
              {group.label}
            </div>
      ...
      {ips.length === 0 && nets.length === 0 && uas.length === 0 && (
        <div className="bg-zinc-900 border border-zinc-800 rounded-lg px-3 py-6 text-[11px] text-zinc-500 text-center">
          No entries yet.
        </div>
      )}
    </div>
