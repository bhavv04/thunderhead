  ...
  return (
    <div className="h-full overflow-y-auto p-3 flex flex-col gap-2.5">
      ...
      <div className="grid grid-cols-2 gap-2">
        <Panel title="Score distribution" right="0–100">
            <ScoreHeatmap logs={logs} />
        </Panel>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
        ...
        <Panel title="Top IPs by request volume">
          <div className="flex flex-col gap-2">
            {topIPs.length === 0 && <div className="text-xs text-zinc-500">Waiting for data…</div>}
            {topIPs.map(([ip, n]) => {
      ...
      <Panel title="Request scatter" right="score over time">
            <ScoreScatter logs={logs} />
        </Panel>
    </div>
  );
}